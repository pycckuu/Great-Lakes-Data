...
idx=1
  ...
  river = "Raisin_River"
  file_loc = paste("/Users/imarkelo/git/Great-Lakes-Data/post_proc_scripts/Loadings US/One_by_one/Western/US/Raisin_R/", sep="")
    sample_name = paste(species, "_", river, "_data.csv", sep = "")
    Daily <- readUserDaily(file_loc, "Flow_Raisin_R_CFS.csv", qUnit = 1)
    Sample <- readUserSample(file_loc, "TP_Raisin_River_data.csv")
