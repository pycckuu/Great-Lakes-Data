...
idx=1
  ...
  river = "Buffalo_River"
  file_loc = paste("/Users/imarkelo/git/Great-Lakes-Data/post_proc_scripts/Loadings US/One_by_one/Western/US/Buffalo_R/", sep="")
    sample_name = paste(species, "_", river, "_data.csv", sep = "")
    Daily <- readUserDaily(file_loc, "Flow_Buffalo_R_CFS.csv", qUnit = 1)
    Sample <- readUserSample(file_loc, "TP_Buffalo_River_data.csv")
